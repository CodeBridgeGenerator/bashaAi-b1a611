...
import ChatEmbeddingsDatatable from "./ChatEmbeddingsDataTable";
...
import ChatEmbeddingsFakerDialogComponent from "./ChatEmbeddingsFakerDialogComponent";
import ChatEmbeddingsSeederDialogComponent from "./ChatEmbeddingsSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const ChatEmbeddingsPage = (props) => {
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("chatEmbeddings");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , userId : urlParams.singleUsersId ,conversationId : urlParams.singleConversationsId ,messageId : urlParams.singleMessagesId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Chat Embeddings", type: "error", message: error.message || "Failed get Chat Embeddings" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("chatEmbeddings").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Chat Embeddings", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("chatEmbeddings").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/chatEmbeddings/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="chatEmbeddings-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="chatEmbeddings-datatable">
                    <ChatEmbeddingsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <ChatEmbeddingsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <ChatEmbeddingsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <ChatEmbeddingsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <ChatEmbeddingsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(ChatEmbeddingsPage);
