...
import chatEmbeddingsFakerFactory from "./chatEmbeddingsFakerFactory";
...
const ChatEmbeddingsFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [userIdIds, setuserIdIds] = useState();
const [conversationIdIds, setconversationIdIds] = useState();
const [messageIdIds, setmessageIdIds] = useState();
...
client.service("conversations")
                        .find({ query: { $select: ["_id","title"] } })
                        .then((data) => {
                        setconversationIdIds(data.data);
                        });
client.service("messages")
                        .find({ query: { $select: ["_id","content"] } })
                        .then((data) => {
                        setmessageIdIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = chatEmbeddingsFakerFactory(props.user,fakerCount,userIdIds,conversationIdIds,messageIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("chatEmbeddings").create(elem));
...
export default connect(mapState, mapDispatch)(ChatEmbeddingsFakerDialogComponent);
