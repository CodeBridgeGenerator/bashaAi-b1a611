...
const ChatEmbeddingsEditDialogComponent = (props) => {
...
messageId: _entity?.messageId?._id,
content: _entity?.content,
embedding: _entity?.embedding,
        };

        setLoading(true);
        try {
            
        await client.service("chatEmbeddings").patch(_entity._id, _data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info chatEmbeddings updated successfully" });
...
const messageIdOptions = messageId.map((elem) => ({ name: elem.name, value: elem.value }));
    ...
    return (
        <Dialog header="Edit Chat Embeddings" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="chatEmbeddings-edit-dialog-component">
...
export default connect(mapState, mapDispatch)(ChatEmbeddingsEditDialogComponent);
