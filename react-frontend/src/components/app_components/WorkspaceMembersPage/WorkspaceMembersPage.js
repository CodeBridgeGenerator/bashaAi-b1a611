...
import WorkspaceMembersDatatable from "./WorkspaceMembersDataTable";
import WorkspaceMembersEditDialogComponent from "./WorkspaceMembersEditDialogComponent";
import WorkspaceMembersCreateDialogComponent from "./WorkspaceMembersCreateDialogComponent";
import WorkspaceMembersFakerDialogComponent from "./WorkspaceMembersFakerDialogComponent";
import WorkspaceMembersSeederDialogComponent from "./WorkspaceMembersSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const WorkspaceMembersPage = (props) => {
    ...
    const filename = "workspaceMembers";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("workspaceMembers");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , workspaceId : urlParams.singleWorkspacesId ,userId : urlParams.singleUsersId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "WorkspaceMembers", type: "error", message: error.message || "Failed get WorkspaceMembers" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("workspaceMembers").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "WorkspaceMembers", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("workspaceMembers").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/workspaceMembers/${data._id}`);
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>WorkspaceMembers </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="workspaceMembers-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="workspaceMembers-datatable">
                    <WorkspaceMembersDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <WorkspaceMembersEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <WorkspaceMembersCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <WorkspaceMembersFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <WorkspaceMembersSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(WorkspaceMembersPage);
