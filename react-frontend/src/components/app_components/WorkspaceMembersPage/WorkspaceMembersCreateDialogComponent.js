...
const WorkspaceMembersCreateDialogComponent = (props) => {
...
const [userId, setUserId] = useState([])
    ...
    useEffect(() => {
        ...
        if (!_.isEmpty(props?.entity)) {
            init = initilization({ ...props?.entity, ...init }, [workspaceId,userId], setError);
        }
        set_entity({...init});
        setError({});
    }, [props.show]);
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            workspaceId: _entity?.workspaceId?._id,userId: _entity?.userId?._id,role: _entity?.role,
        ...
        const result = await client.service("workspaceMembers").create(_data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info WorkspaceMembers updated successfully" });
        props.onCreateResult(eagerResult.data[0]);
        } catch (error) {
            console.debug("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in WorkspaceMembers" });
        }
        setLoading(false);
    };
...
const userIdOptions = userId.map((elem) => ({ name: elem.name, value: elem.value }));
    ...
    return (
        <Dialog header="Create WorkspaceMembers" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="workspaceMembers-create-dialog-component">
...
export default connect(mapState, mapDispatch)(WorkspaceMembersCreateDialogComponent);
