...
import workspaceMembersFakerFactory from "./workspaceMembersFakerFactory";
...
const WorkspaceMembersFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [workspaceIdIds, setworkspaceIdIds] = useState();
const [userIdIds, setuserIdIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("workspaces")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setworkspaceIdIds(data.data);
                        });
client.service("users")
    ...
    const onRun = async () => {
        let fakeData = workspaceMembersFakerFactory(props.user,fakerCount,workspaceIdIds,userIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("workspaceMembers").create(elem));
...
export default connect(mapState, mapDispatch)(WorkspaceMembersFakerDialogComponent);
