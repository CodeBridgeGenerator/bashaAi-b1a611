...
import messagesFakerFactory from "./messagesFakerFactory";
...
const MessagesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = messagesFakerFactory(props.user,fakerCount,conversationIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("messages").create(elem));
...
export default connect(mapState, mapDispatch)(MessagesFakerDialogComponent);
