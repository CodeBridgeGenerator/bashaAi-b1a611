...
import conversationsFakerFactory from "./conversationsFakerFactory";
...
const ConversationsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = conversationsFakerFactory(props.user,fakerCount,userIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("conversations").create(elem));
...
export default connect(mapState, mapDispatch)(ConversationsFakerDialogComponent);
