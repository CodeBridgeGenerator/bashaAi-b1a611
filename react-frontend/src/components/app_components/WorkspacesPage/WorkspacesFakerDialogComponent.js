...
import workspacesFakerFactory from "./workspacesFakerFactory";
...
const WorkspacesFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [ownerIdIds, setownerIdIds] = useState();
    ...
    const onRun = async () => {
        let fakeData = workspacesFakerFactory(props.user,fakerCount,ownerIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("workspaces").create(elem));
...
export default connect(mapState, mapDispatch)(WorkspacesFakerDialogComponent);
