...
const WorkspacesDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const dropdownTemplate1 = (rowData, { rowIndex }) => <p >{rowData.ownerId?.name}</p>
...
<Column field="ownerId" header="Owner Id" body={dropdownTemplate1} filter={selectedFilterFields.includes("ownerId")} hidden={selectedHideFields?.includes("ownerId")}  style={{ minWidth: "8rem" }} />
      ...
      {selectedItems.length > 0 ? (
        ...
        <Dialog header="Upload Workspaces Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
      ...
      <Dialog header="Search Workspaces" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default WorkspacesDataTable;