...
import WorkspacesDatatable from "./WorkspacesDataTable";
import WorkspacesEditDialogComponent from "./WorkspacesEditDialogComponent";
import WorkspacesCreateDialogComponent from "./WorkspacesCreateDialogComponent";
import WorkspacesFakerDialogComponent from "./WorkspacesFakerDialogComponent";
import WorkspacesSeederDialogComponent from "./WorkspacesSeederDialogComponent";
...
const WorkspacesPage = (props) => {
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("workspaces");
    ...
    useEffect(() => {
        ...
        client
            .service("workspaces")
            .find({ query: { $limit: 10000 , ownerId : urlParams.singleUsersId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("workspaces").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Workspaces", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("workspaces").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="workspaces-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="workspaces-datatable">
                    <WorkspacesDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <WorkspacesEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <WorkspacesCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <WorkspacesFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <WorkspacesSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(WorkspacesPage);
